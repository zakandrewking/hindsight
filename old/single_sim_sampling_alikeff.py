...
from me_scripts.sampling import generate_sampling_problems, get_nondefault_enzyme_efficiency
...
def load_problems():
    ...
    with open(join(setup_dir, 'alikeffs.json'), 'r') as f:
        alikeffs = json.load(f)
    # with open('14..08.04_keff_fits.json', 'r') as f:
    #     keff_fits = json.load(f)
    # set up the sims
    n = '14..08.04_Jantama2008_sampling_lognorm_alikeff'
    ...
    for setup in setups:
        ...
        nondefault_enzyme_efficiency = get_nondefault_enzyme_efficiency()
        nondefault_enzyme_efficiency.update(alikeffs)
        sim = EZ_Batch_Maximum_Growth(data_dir,
                                      ...
                                      nondefault_enzyme_efficiency=nondefault_enzyme_efficiency)
        sims = generate_sampling_problems([sim], 500, distribution='lognormal',
                                          center_for_mu=True, start_with=0)
        return sims
